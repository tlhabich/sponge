...
protected:
    ...
    int default_decimation_ = 20; // Reduktion der an den nicht-Echtzeitbereich zu sendenden Daten
    int publish_decimation_value_; //TODO decimation value ueber parameter server auslesen
    bool is_stopped_;
        ...
        was_acquired_(false) // Start ohne Verbindung zu Client
{
        ...
        sub_gpr_ = nh_.subscribe("/SL_RT_CORE/gprOut", 1, &SLNode::sub_gpr_cb, this);
        // Initialisiere ROS-Publisher
        pub_q_meas_ = nh_.advertise<std_msgs::Float64MultiArray>("q_meas", 1);
        pub_qd_meas_ = nh_.advertise<std_msgs::Float64MultiArray>("qd_meas", 1);
        pub_test_ = nh_.advertise<std_msgs::UInt8>("test", 1);
        pub_datapoint_ = nh_.advertise<std_msgs::Float64MultiArray>("datapoint", 1);
        pub_controller_state_ = nh_.advertise<pcu_sl_interface::ControllerState>("controller_state", 1, true);
        // Parametriere Publisher
        nh_.param("SL_RT_CORE/publish_decimation", publish_decimation_value_, default_decimation_);
    ...
    void run(){
        ...
        while (ros::ok() && !is_stopped_)
        {
            ...
            std_msgs::Float64MultiArray q_meas;
            std_msgs::Float64MultiArray qd_meas;
            std_msgs::Float64MultiArray datapoint;
            std_msgs::UInt8 test;
            ...
            std::vector<double> Q_meas((2), 0.0);
            std::vector<double> Qd_meas((2), 0.0);
            std::vector<double> DataPoint((11), 0.0);
            
            for (int k = 0; k != DataPoint.size(); k++) {
                DataPoint.at(k) = sl_out_buffer_.datapoint[k];
            }
            
            datapoint.layout.dim.push_back(std_msgs::MultiArrayDimension());
            datapoint.layout.dim[0].size = DataPoint.size();
            datapoint.layout.dim[0].stride=1;
            datapoint.layout.dim[0].label = "datapoint";
           
            
            datapoint.data.clear();
            datapoint.data.insert(datapoint.data.end(), DataPoint.begin(), DataPoint.end());
            
            
            Var_Test = sl_out_buffer_.test;
            test.data = Var_Test;
            
            Q_meas.at(0) = sl_out_buffer_.q_meas[0];
            Q_meas.at(1) = sl_out_buffer_.q_meas[1];
            
            Qd_meas.at(0) = sl_out_buffer_.qd_meas[0];
            Qd_meas.at(1) = sl_out_buffer_.qd_meas[1];
            
            qd_meas.layout.dim.push_back(std_msgs::MultiArrayDimension());
            qd_meas.layout.dim[0].size = Qd_meas.size();
            qd_meas.layout.dim[0].stride=1;
            qd_meas.layout.dim[0].label = "qd_meas";
            
            q_meas.layout.dim.push_back(std_msgs::MultiArrayDimension());
            q_meas.layout.dim[0].size = Q_meas.size();
            q_meas.layout.dim[0].stride=1;
            q_meas.layout.dim[0].label = "q_meas";
            
            q_meas.data.clear();
            q_meas.data.insert(q_meas.data.end(), Q_meas.begin(), Q_meas.end());
            
            qd_meas.data.clear();
            qd_meas.data.insert(qd_meas.data.end(), Qd_meas.begin(), Qd_meas.end());
            ...
            if(decimation_counter==0){
                ...
                pub_datapoint_.publish(datapoint);
            }
...
private:
    ...
    void sub_gpr_cb(const std_msgs::Float64MultiArray::ConstPtr& msg)
    {
        // Callback-Funktion für Subscriber von gpr
        // Wird aufgerufen, wenn neue ROS-Message dafür ankommt
        //ROS_WARN("TEST");
        pthread_mutex_lock(&mut_in_); // Sperre Zugriff auf sl_in_buffer_
        // Daten von ROS-Message in Buffer schreiben
        for(int k = 0; k != 2500; k++) {
            sl_in_buffer_.gpr[k] = msg->data[k];
        }
        pthread_mutex_unlock(&mut_in_); // Freigabe von sl_in_buffer_
    }
